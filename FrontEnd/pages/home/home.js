...
const _HOME_CACHE_TTL_MS = 45000

function _homeHasFreshCache() {
  return _homeDataLoadedAt > 0
    && (Date.now() - _homeDataLoadedAt) < _HOME_CACHE_TTL_MS
    && _homeData
...
function _homeSetFeedback(failedSources = []) {
  ...
  el.innerHTML = `
    ...
    <div class="section-feedback-sub">Alguns módulos não responderam nesta tentativa.</div>
    <div class="section-feedback-details">${failedSources.join(' · ')}</div>
  `

  if (typeof showAppToast === 'function') {
    showAppToast('Home carregada parcialmente. Verifique a conexão.', 'error')
  }
}

async function initHomeSection(forceRefresh = false) {
  _homeRenderGreeting()
  ...
  if (!forceRefresh && _homeHasFreshCache()) {
    _homeRenderAll()
    return
  }
  ...
  const failedSources = []
  if (checkinsR.status !== 'fulfilled') failedSources.push('Body')
  if (exR.status !== 'fulfilled') failedSources.push('Exercises')
  if (goalsEntR.status !== 'fulfilled' || goalsMetR.status !== 'fulfilled') failedSources.push('Goals')
  if (finOk && (lancR.status !== 'fulfilled' || codR.status !== 'fulfilled' || orcR.status !== 'fulfilled')) {
    failedSources.push('Finances')
  }
  _homeSetFeedback(failedSources)
    ...
    <div class="hcard-metrics-grid">
      ${secCards.map(m => `
        <div class="hcard-metric">
          ...
          ${m.info.dAno !== null ? `
          <div class="hcard-delta-row hcard-delta-row-tight">
