...
let _exRenderRAF = null
let _exBarScrollTimer = null
...
function renderExChips() {
  ...
  _setHidden(container, chips.length === 0)
}
...
async function saveExercise() {
  ...
  if (!cdExercicio) {
    if (typeof showAppToast === 'function') showAppToast('Selecione um exercício.', 'error')
    return
  }
  if (!hora) {
    if (typeof showAppToast === 'function') showAppToast('Informe a hora.', 'error')
    return
  }
  if (!data) {
    if (typeof showAppToast === 'function') showAppToast('Informe a data.', 'error')
    return
  }
...
function renderExDash() {
  if (_exRenderRAF != null) return
  _exRenderRAF = requestAnimationFrame(() => {
    _exRenderRAF = null
    _renderExDashNow()
  })
}

function _renderExDashNow() {
...
function renderExChartBar(base) {
  ...
  if (_exBarScrollTimer) clearTimeout(_exBarScrollTimer)
  _exBarScrollTimer = setTimeout(() => {
    if (!isDrill) {
      if (exCross.mes) {
...
function destroyExerciseCharts() {
  if (_exRenderRAF != null) {
    cancelAnimationFrame(_exRenderRAF)
    _exRenderRAF = null
  }
  if (_exBarScrollTimer) {
    clearTimeout(_exBarScrollTimer)
    _exBarScrollTimer = null
  }
  if (exChartBar) {
    exChartBar.destroy()
  ...
  if (exChartRosca) {
    exChartRosca.destroy()
    exChartRosca = null
  }
  if (exChartPeriodo) {
    exChartPeriodo.destroy()
    exChartPeriodo = null
  }
}

window.destroyExerciseCharts = destroyExerciseCharts
