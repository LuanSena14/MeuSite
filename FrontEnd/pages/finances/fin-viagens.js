...
function _viagDateKey(dateRaw) {
  if (!dateRaw) return Number.NEGATIVE_INFINITY
...
function _viagLatestDate(v) {
  if (v?.ultima_data) return String(v.ultima_data).slice(0, 10)
...
function _sortViagensLatestDesc(arr) {
  return arr.slice().sort((a, b) => {
    const aKey = _viagDateKey(_viagLatestDate(a))
    const bKey = _viagDateKey(_viagLatestDate(b))
    if (bKey !== aKey) return bKey - aKey
    return String(a?.nome_viagem || '').localeCompare(String(b?.nome_viagem || ''), 'pt-BR')
  })
}
...
async function renderViagens() {
  ...
  // Ordena por último lançamento real da viagem (mais recente no topo)
  const viagens = _sortViagensLatestDesc(window.finViagens)
  ...
  list.innerHTML = viagens.map((v, idx) => {
    ...
    const ultimaData = _viagLatestDate(v)
    const mesAno     = ultimaData
      ? new Date(ultimaData + 'T12:00:00').toLocaleDateString('pt-BR', { month: 'short', year: '2-digit' })
      : '—'
...
function _buildViagBar() {
  const viagens = _sortViagensLatestDesc(_viagAll)
