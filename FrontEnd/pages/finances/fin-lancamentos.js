...
function renderOrcamento() {
  ...
  if (anuais.length > 0) {
    html += `<div class="fin-orc-section-label fin-orc-section-label-top">Base anual</div>`
    html += _buildOrcGroupHtml(_buildOrcTree(anuais, realizado), true, 'tab-a')
  }
  if (!html) html = '<p class="inline-empty-note-center">Nenhum orçamento para o período.</p>'
...
function _renderRecorrentePanel(ano, mes, mesLabel, realizadoMap) {
  ...
  container.innerHTML = `
    <div class="fin-rec-layout">
      ${summaryHtml}
      <div class="fin-rec-donos">${typeColsHtml || '<p class="inline-empty-note-pad">Sem dados recorrentes para o período.</p>'}</div>
    </div>`
}
...
function _renderCreditoPanel(ano, mes, mesLabel) {
  ...
  if (!allIds.length) {
    container.innerHTML = `
      <div class="dash-card fin-block-mt16">
        <div class="fin-row-between">
          <div class="dash-card-title fin-title-credit">Cartão de Crédito — ${mesLabel}</div>
          <button class="btn-add" onclick="openFinModal('orcamento')">+ Orçamento</button>
        </div>
        <p class="inline-empty-note">Nenhum lançamento de crédito no período.</p>
      </div>`
    return
  }
  ...
  container.innerHTML = `
    <div class="dash-card fin-block-mt16">
      <div class="fin-row-between" style="margin-bottom:10px">
        <div class="dash-card-title fin-title-credit">Cartão de Crédito — ${mesLabel}</div>
        <button class="btn-add" onclick="openFinModal('orcamento')">+ Orçamento</button>
      </div>
      <div class="fin-row-baseline-gap">
        <span class="fin-credit-total">${_fmtBRL(totalReal)}</span>
        ${totalOrc > 0 ? `<span class="fin-credit-sub">de ${_fmtBRL(totalOrc)} orçado — ${((totalReal / totalOrc) * 100).toFixed(0)}%</span>` : ''}
      </div>
      ${totalOrc > 0 ? `<div class="fin-orc-bar-bg fin-credit-bar-gap"><div class="fin-orc-bar-fill ${over ? 'over' : ''}" style="width:${barPct}%"></div></div>` : ''}
      <table class="fin-rec-cat-table">
